...
function Kx_XHR_Send(data, before, oncomplete, onsuccess, onerror, options){
    ...
    try {
        ...
        if ( window.isUnloading === true ) {
            console.log("NOTICE : Your asynchronous request has been aborted due to an unloading event");
            return;
        }
...
//        Kxlib_DebugVars(["EXCEPTION : [MESSAGE] : "+ex+"; [LINE] : "+ex.lineNuumber]); //[DEPUIS 29-08-15]
...
//        var _Obj = window.AjaxRules;
        ...
        var o = ( !KgbLib_CheckNullity(uz) ) ? AJAXRULES(uq,uz) : AJAXRULES(uq);
...
a//        Kxlib_DebugVars([ex,ex.fileName,ex.lineNumber]);
    }
}
...
function Kxlib_GetTrendPropIfExist () {
    ...
    if ( $("meta[property='tq:ufn']").length ) {
        var r = {
            ...
            "trttl"     : $("meta[property='tq:trtitle']").attr("content"),
...
function Kxlib_AjaxGblOnErr(xhr,ts) {
    ...
    switch (xhr.status) {
        ...
        case 401:
                ...
                KD.title = null;
                KD.message = "Vous n'êtes sans doute pas autorisé à effectuer cette action. Veuillez recharger la page, pour tenter de résoudre le problème.";        
                KD.valid = {
                    text        : "Recharger",
                    actionType  : "reload",
                    action      : null
...
//        Kxlib_DebugVars([gbLib_CheckNullity(s), typeof s !== "string", KgbLib_CheckNullity(p), !$.isArray(p)]);
    ...
    $.each(p,function(x,v){
        /*
         * [DEPUIS 18-11-15]
         *      Laisser le choix à l'utilisateur d'ouvrir dans un nouvel onglet. 
         *      On ne renvoie vers un autre onglet que dans le cas des URLICS
         */
//        ns__ = ns__.replace(new RegExp("\@"+v+"","gi"),"<a class='"+c+"' href='/"+v.toLowerCase()+"' target='_blank'>@"+v+"</a>");
//        ns__ = ns__.replace(new RegExp("\@"+v+"","gi"),"<a class='"+c+"' href='/"+v.toLowerCase()+"'>@"+v+"</a>");
        /*
         * [DEPUIS 11-08-16]
         */
        var rgx = "@("+v+")(?![\\wÀÁÂÃÄÅÆàáâãäåæắạẶặẰằẢảÞßþÇçĐđƉɖÐðÈÉÊËèéêëềệĘęÌÍÎÏìíîïłÒÓÔÕÖØΘòóôõöơởøÑñⱣᵽÙÚÛÜùúûüựÿÝýŠŽžż])";
//        ns__ = ns__.replace(new RegExp(rgx,"gi"),"<a class='"+c+"' href='/"+v.toLowerCase()+"'>@"+v+"</a>");
        ns__ = ns__.replace(new RegExp(rgx,"gi"),"<a class='"+c+"' href='/"+v.toLowerCase()+"'>@"+v+"</a>");
    });
    return ns__;
}
...
function Kxlib_SplitByUsertags (s) {
    ...
    *      On ne renvoie vers un autre onglet que dans le cas des URLICS
    */
    var ptrn = /(<a class='(?:tqr-unq-user|tqr-arp-user)' href='\/\@?[\w]+'>(\@[\w]+)<\/a>)/g;
...
/*
//var _t_ = "Un texte avec 4 liens : (1) http://trenqr.com; (2) trenqr.me; (3) trenqr.me; (4) https://www.trenqr.us";
var _t_ = "Un message avec plusieurs liens : www.trenqr.com; http://trenqr.com //trenqr.com trenqr.me; trenqr.com .trenqr.com et un hack &lt;script&gt;alert('123456789')&lt;/script&gt;";
//var _t_ = "trenqr.com";
var _us_ = [
    {
        'uicid'     : "urleid1",
        'uic_url'  : "http://trenqr.com",
        'uic_cid'   : "aeid1"
    },
    {
        'uicid'     : "urleid2",
...
//*/
//    Un message avec plusieurs liens : www.trenqr.com; trenqr.me; trenqr.com et un hack &lt;script&gt;alert('123456789')&lt;/script&gt;
function Kxlib_ActivateURLIC (t,us) {
    //addClass, addMetas
    try {
        if ( KgbLib_CheckNullity(t) | KgbLib_CheckNullity(us) ) {
            return;
        }

        var tx = Kxlib_Decode_After_Encode(t);    
        /*
         * ETAPE :
         *      Pour chaque URLIC dans le tableau on remplace dans le texte.
         */
        var aldy = spltr = [];
        $.each(us,function(x,utb){
            if ( ( aldy && $.inArray(utb.uic_url,aldy) !== -1 ) ) {
                return true;
            }
            var hrf = ( /^https?:\/\//i.test(utb.uic_url) ) ? utb.uic_url : "http://"+utb.uic_url;
            var a = $("<a/>",{
                "data-ds"   : JSON.stringify(utb),
                "data-item" : utb.uicid,
                "target"    : '_blank',
                "href"      : hrf,
                "class"     : 'tqr-uic-a jb-tqr-uic-a'
            }).data("ds",JSON.stringify(utb)).text(utb.uic_url);
            var atx = $("<div/>").append($(a).clone()).html();
            
            var re;
            if ( /^www./i.test(utb.uic_url) || /^https?:\/\//i.test(utb.uic_url) ) {
                re = new RegExp(utb.uic_url,'g');
                tx = tx.replace(re,atx);
            } else {
                /*
                 * [NOTE]
                 *      Les regex sont le fruit d'une LONGUE et FASTIDIEUSE série d'expérimentations pour espérer gérér au moins 90% des cas.
                 *      S'il faut les modifier, prennez patience, ne perdez pas espoir, persévérez !
                 */
//                (?:((?:(?:\\s|^$)|(?:[^a-z\\/]{2,}))\\.?)("+utb.uic_url+"))|(?:^("+utb.uic_url+"))
//                re = new RegExp("(?:((?:(?:\\s|^$)|(?:[^a-z\\/]{2,}))\\.?)("+utb.uic_url+"))|(?:^("+utb.uic_url+"))","gi");

                re = new RegExp("(?:((?:(?:\\s|^$)|(?:[^a-z\\/]{2,}))\\.?)("+utb.uic_url+"\\/?)(?!(?:[a-z\\d])|(?:\\/[^\\s])))|(?:^("+utb.uic_url+"\\/?)(?!(?:[a-z\\d])|(?:\\/[^\\s])))","gi");
//                re = new RegExp("(?:\\s|^$)\.?("+utb.uic_url+")","gi");

//                Kxlib_DebugVars([tx,re,re.test(tx)],true);

                tx = tx.replace(re,function(a,b,c,d){
//                    console.log(a,'#',b,'#',c,"#",d);
                    return ( b ) ? b+atx : atx;
                });
            }
...
/*
var yo = Kxlib_ActivateURLIC(_t_,_us_);
$(".TEST-ZONE").html(yo);
//*/
/**
 * Permet de récupérer le premier ou le dernier élément de la liste, triée par une propriété de type 'time'.
 * 
 * @depuis 06-09-15
 * @author Lou Carther
 * @param {array} gsl Le groupe des éléments de type DOM
 * @param {string} prop La propriété qui servira de pivot dans la recherche
 * @param {string} caz Le cas voulu (FIRST, LAST)
 * @returns {Boolean|undefined|Number}
 */
function Kxlib_GetFirstDomSortedByTimeBy (gsl,prop,caz) {
    
    if ( KgbLib_CheckNullity(gsl) | KgbLib_CheckNullity(prop) | KgbLib_CheckNullity(caz) ) {
        return;
    }
    ...
    if (! $(gsl).filter("["+prop+"]").length ) {
        return false;
    }
    if ( $.inArray(caz.toUpperCase(),["FIRST","LAST"]) === -1 ) {
        return false;
    }
    
    var srt_lst = [];
    $.each($(gsl).filter("["+prop+"]"),function(x,e){
       if ( typeof parseInt($(e).attr(prop)) === "number" ) {
           srt_lst.push([e,parseInt($(e).attr(prop))]);
           /*
            * On trie par ordre décroissant
            */
           srt_lst.sort(function(a, b) {return a[1] - b[1];});
       } 
    });
    
    if ( KgbLib_CheckNullity(srt_lst) ) {
        return -1;
    } else if ( caz.toUpperCase() === "LAST" ) {
        return srt_lst[0][0];
    } else {
        var ln__ = srt_lst.length;
        return srt_lst[--ln__][0];
    }
    
}

function Kxlib_FstLetterCap (s) {
    return s.charAt(0).toUpperCase()+s.slice(1);
}

function Kxlib_SetDataWith (ustgs) {
    try {
        if ( KgbLib_CheckNullity(ustgs) ) {
            return;
        }
        
        var str__;
        if ( !KgbLib_CheckNullity(ustgs) && ustgs !== undefined && typeof ustgs === "object" ) {
            var istgs__ = [];
            $.each(ustgs, function(x, v) {
                var rw__ = [];
                $.map(v, function(e,x) {
                    rw__.push(e);
                });
...
/*
 * Permet de traiter un texte brut en le rendant RICH. Voici les caractéristiques necessaires : 
 *  -> Le texte est sécurisé
 *  -> Le texte interprète les USERTAGS
 *  -> Le texte interprète les HASHTAGS
 *  -> Le texte interprète les URLS
 *  -> Le texte interprète les Emojis
 */
function Kxlib_TextEmpow (t,us,ht,ul,opts) {
    try {
         ...
         *      
         *  [NOTE 17-02-16]
         *      Cette portion de code a été récupérée. Elle suit une logique "étrangère".
         */
//        nt = t;
//        nt = $("<div/>").text(t).html();
        nt = $('<textarea />').html(t).text();
        
        if ( !KgbLib_CheckNullity(us) ) {
            us_cz = true;
            var str__ = Kxlib_SetDataWith(us);
...
//            Kxlib_DebugVars([Kxlib_ObjectChild_Count(v.austgs),ustgs[3]],true);
            ...
            var us_str = Kxlib_UsertagFactory(nt, ps, "tqr-unq-user");
            
            /*
             * [DEPUIS 28-07-16]
             *      Boguait avec les PSEUDO contenant des caractères accentués
             */
//            us_str = Kxlib_SplitByUsertags(us_str);
            emt = $("<div/>").html(us_str);
//            emt = nt;
        }
...
//        return emt;
         ...
         *      
         */
        if ( !KgbLib_CheckNullity(ht) ) {
            nt = ( emt ) ? $(emt).html() : nt;
            $.each(ht,function(x,h){
                var c = "tqr-unq-user";
                nt = nt.replace(new RegExp("\#"+h+"(?![a-z\d])","gi"),"<a class='"+c+"' href='/hview/q="+h.toLowerCase()+"&src=hash'>#"+h+"</a>");
            });
        ...
        /*
         * ETAPE :
         *      On RENDER les liens en fonction des instructions passées en paramètre.
         */
        if ( opts && opts.hasOwnProperty("ena_inner_link") && !KgbLib_CheckNullity(opts.ena_inner_link.all) && opts.ena_inner_link.all === true ) {
            
        } else if ( opts && opts.hasOwnProperty("ena_inner_link") && !KgbLib_CheckNullity(opts.ena_inner_link.only) && opts.ena_inner_link.only === "fksa" ) {
            nt = ( emt ) ? $(emt).html() : nt;
            
            var uls = ( !KgbLib_CheckNullity(opts.ena_inner_link.local) && opts.ena_inner_link.local === true ) ? 
                nt.match(/(?:https?:\/\/)?127.0.0.1\/f(?:(?:\/sts)|(?:\/vid))?\/[\w\-_]{5,50}/gi)
                : nt.match(/(?:https?:\/\/)?(?:(?:www\.)|(?:beta\.)|(?:dm1606\.))?trenqr\.com\/f(?:(?:\/sts)|(?:\/vid))?\/[\w\-_]{5,50}/gi);
            
            if ( uls ) {
                var aldy = [];
                $.each(uls,function(x,ul){
                    if ( aldy && $.inArray(ul,aldy) !== -1 ) {
                        return true;
                    }
                    aldy.push(ul);
                    
//                    console.log("RENDER INNER LINK (RAW) : ".concat(ul));
                    var hrf = ( (new RegExp("^https?:\/\/","gi")).test(ul) ) ? ul : "http://".concat(ul);
                    var rgx = "^("+ul+")|[^(?:https?:\/\/)|(?:www\.)|(?:a-z\d)|(?:href=')]("+ul+")";
                    
//                    console.log("RENDER INNER LINK (HREF) : ".concat(hrf));
//                    nt = nt.replace(new RegExp(""+ul+"","gi"),"<a class='tqr-inner-link' href='"+hrf+"'>"+ul+"</a>");
//                    nt = nt.replace(new RegExp("[^(?:https?:\/\/)|(?:www\.)|(?:a-z\d)|(?:href=')]("+ul+")","gi"),"<a class='tqr-inner-link' href='"+hrf+"'>"+ul+"</a>");
                    nt = nt.replace(new RegExp(rgx,"gi"),function(a,b){
                        return ( a === b ) 
                            ? "<a class='tqr-inner-link' href='"+hrf+"'>"+ul+"</a>" 
                            : a.substring(0,1)+"<a class='tqr-inner-link' href='"+hrf+"'>"+ul+"</a>";
                    });
//                    console.log("RENDER INNER LINK (SORTIE) : ".concat(nt));
                });
                emt = $("<div/>").html(nt);
            } 
        }
        ...
        /*
         * ETAPE :
         *      On transforme en EMOJI les caractères éligibles
         */
//        console.log(">>> ",t,">>> ",nt,">>> ",$(emt).html());
/*
        var eml_rgx = ( us_cz ) ? /(&amp;#x)[\dA-Z]{4,5};/gi : /(&#x)[\dA-Z]{4,5};/gi;
//        return;
        if ( nt.match(eml_rgx) ) {
            t__ = nt.replace(eml_rgx,function(p1){
//                console.log(p1);
                var t_ = $("<div/>").html(p1).text();
//                console.log(t_);
                return $("<div/>").html(t_).text();
            });
            t__ = twemoji.parse(t__,{
                callback : function(icon, options) {
                  return '//twemoji.maxcdn.com/' + options.size + '/' + icon + '.png';
                },
                size: ( opts && opts.hasOwnProperty("emoji") 
                        && opts.emoji.hasOwnProperty("size") && opts.emoji.size 
                        && $.inArray(parseInt(opts.emoji.size),[16,36,72,128]) 
                ) ? opts.emoji.size : 16
            });
...
//            emt = $("<span/>").html(t__);
            ...
            if ( 
                    opts && opts.hasOwnProperty("emoji") 
                    && opts.emoji.hasOwnProperty("size_css") && opts.emoji.size_css
                ) 
            {
                $(emt).find(".emoji").css({
                    height : opts.emoji.size_css.toString().concat("px")
                });
            }
...
//        emt = Kxlib_ParseEmoji(nt,opts);
        var t__ = ( emt ) ? $("<div/>").html(emt).html() : nt;
        emt = Kxlib_ParseEmoji(t__,opts);

        emt = (! emt ) ? $("<div/>").text(nt).text() : emt;
        
        /*
         * [DEPUIS 14-04-16]
         */
//        Kxlib_DebugVars([!emt,opts,opts.hasOwnProperty("wrap_text"),opts.wrap_text === true],true);
        emt = ( typeof emt === "string"  && opts && opts.hasOwnProperty("wrap_text") && opts.wrap_text === true ) ?
        $("<div/>").html(emt) : emt;
...
function Kxlib_ParseEmoji (t,opts) {
    try {
        if ( KgbLib_CheckNullity(t) ) {
            return;
        }
        
        var t__ = twemoji.parse(t,{
            callback : function(icon, options) {
              return '//twemoji.maxcdn.com/' + options.size + '/' + icon + '.png';
            },
            size: ( opts && opts.hasOwnProperty("emoji") 
        ...
        var pos_x = ( opts && opts.hasOwnProperty("emoji") && opts.emoji.hasOwnProperty("position_x") && opts.emoji.position_x ) ? opts.emoji.position_x.toString().concat("px") : "auto";
        var pos_y = ( opts && opts.hasOwnProperty("emoji") && opts.emoji.hasOwnProperty("position_y") && opts.emoji.position_y ) ? opts.emoji.position_y.toString().concat("px") : "auto";
        var r = t__;
        if ( 
                opts && opts.hasOwnProperty("emoji") 
                && opts.emoji.hasOwnProperty("size_css") && opts.emoji.size_css
        ...
        {
            if ( typeof r === "string" ) { //RAPPEL : JQuery n'accepte pas les STRING en entré
                r = $("<div/>").html(r);
                $(r).find(".emoji").css({
                    height      : opts.emoji.size_css.toString().concat("px"),
                    position    : "relative",
...
function Kxlib_WindOverflow (hid) {
    try {
        if ( hid ) {
...
function Kxlib_RemTime (fut,now) {
    try {
        if ( KgbLib_CheckNullity(fut) | KgbLib_CheckNullity(now) ) {
            return;
        }
        ...
        var delta = Math.abs(fut - now) / 1000;
...
function Kxlib_ExtractDomain (url) {
    if ( KgbLib_CheckNullity(url) ) {
        return;
    }
...
function Kxlib_GetAbsolutePathFromUrl (url) {
    if ( KgbLib_CheckNullity(url) ) {
        return;
    }
    
    domain = Kxlib_ExtractDomain(url);
...
function Kxlib_DocTitleSwNb (cn) {
    
    var cur_tle = document.title;
    var pcs = cur_tle.match(/^(?:\(\d+\)\s)?(.+)$/i);
    if (! ( pcs && $.isArray(pcs) ) ) {
        alert("No tab");
        return;
    }
    ...
    var ntle;
    if ( KgbLib_CheckNullity(cn) ) {
        ntle = ( pcs[1].indexOf("(") === 0 ) ? pcs[2] : pcs[1];
    } else {
        ntle = document.title = ( pcs[1].indexOf("(") === 0 ) ? "".concat("(",cn,") ",pcs[2]) : "".concat("(",cn,") ",pcs[1]);
    }
    document.title = ntle;
