 ...
 * @author arsphinx
 */
class WORKER_TRPG_GTPG_RO extends WORKER {
...
//            exit();
    ...
    private function AcquireTrendDatas () {
        ...
        $this->presentVarIfDebug(__FUNCTION__, __LINE__, [$this->KDIn["gvn_eid"],$this->KDIn["gvn_title"],$t_tab["trd_title_href"]]);
        if ( !$t_tab || $this->return_is_error_volatile(__FUNCTION__, __LINE__, $t_tab)  ) {
            ...
            $scope = ( $TRD->trend_abo_exists($this->KDIn["oid"],$teid) ) ? "TRPG_GTPG_RFOL" : "TRPG_GTPG_RU";
            ...
            $url = $RDH->redir_build_scoped_url($scope,$args_redir);
            $RDH->start_redir_to_this_url_string($url);
            
            exit(); //PARANO
        } 
        
//        var_dump(__FUNCTION__,__LINE__,$t_tab);
//        exit();
...
//                var_dump(__FUNCTION__,__LINE__,$v["artid"],$a_tab);
...
//            exit();
            ...
            //*/
            /*
            var_dump(__FUNCTION__, __LINE__, ["EAST",$all],'v_d');
//            $this->presentVarIfDebug(__FUNCTION__, __LINE__, ["EAST",$all],'v_d');
...
//            exit();
            /*
             * [DEPUIS 22-04-15]
             * On réunit les données dans un seul et meme tableau.
             * On n'utilise plus le système de 2 colonnes pour des raisons fonctionnelles et de stabilité.
             */
//            array_merge($e,$w);
            //On trie les données par ordre decroissant
            /*usort($as__, function($a,$b){
                return floatval($b["art_crea"]) - floatval($a["art_crea"]);
            });
            //*/
...
//            exit();
            //*/
            /*
            $this->presentVarIfDebug(__FUNCTION__, __LINE__, ["EAST",$this->KDout["articles_east"]],'v_d');
            $this->presentVarIfDebug(__FUNCTION__, __LINE__, ["WEST",$this->KDout["articles_west"]],'v_d');
            exit();
            //*/
        }
    ...
    private function PrepareArticle ( $art_tab ) {
         ...
         */
//        $ouftab = $ART->Favorite_hasFavorite($tgtab["pdaccid"],$art_tab["art_eid"],TRUE);
    ...
    //                "art_desc"      => $art_tab["art_desc"], //ENT_QUOTES,
    //                "art_desc"      => html_entity_decode($art_tab["art_desc"],ENT_QUOTES,"UTF-8"),
    //                "art_desc"      => html_entity_decode($art_tab["art_desc"]),
...
//        var_dump(__LINE__,$_SESSION["sto_infos"]); 
//        exit();
...
//        exit();
    ...
    public function on_process_in() {
         ...
         * [DEPUIS 21-05-16]
         */
        $temp = $_SESSION["rsto_infos"]->getAccid();
//        session_id("6usgdtuj4ujcb3pkprvh4jkii1");
//        session_id("i9i114pbo20a72h1vlh5ktdfd5");
        session_start();
...
//        var_dump($this->KDout["CUser"],$temp,session_id(),$_SESSION["apps"]["ltc"],$_SESSION["rsto_infos"]->getAccid());
//        exit();
    ...
    public function on_process_out() {
        ...
        if ( $this->KDIn["oid"] ) {
            $PM = new POSTMAN();
            $log_r = $this->Wkr_LogUsertagActy($PM, $this->KDIn["oid"], $_SESSION['sto_infos']->getCurrent_ipadd(), $this->KDIn["srv_curl"], $this->KDIn["srv_curl"], FALSE, $this->KDIn["gvn_eid"], 912, TRUE);
        }
