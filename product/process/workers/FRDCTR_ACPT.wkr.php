...
class WORKER_FRDCTR_ACPT extends WORKER  {
...
//        $this->endExecutionIfDebug(__FUNCTION__,__LINE__);
    ...
    private function SetFriendship() {
        //* BUT : Accepter une demande d'amis *//
        ...
        if ( $this->return_is_error_volatile(__FUNCTION__, __LINE__, $rqt) ) {
            ...
            if (! ( ( intval($rqt["rlev_acc_targ"]) === intval($this->KDIn["oid"]) ) && ( intval($rqt["rlev_acc_actor"]) === intval($exists["pdaccid"]) ) ) ) {
                ...
                //On procède à l'acceptation de la demande
                $r = $RL->friend_accept_request($this->KDIn["oid"], $exists["pdaccid"], $rqt["frdrqtid"]);
//                $r = true; //TEST, DEBUG
            }
        } else {
        ...
        if ( $this->return_is_error_volatile(__FUNCTION__, __LINE__, $r) ) {
             ...
             * [DEPUIS 11-05-15] @BOR
             */
            $foo = $RL->onread_relation_exists_fecase($this->KDIn["oid"], $exists["pdaccid"]);
            ...
            /*
             * [DEPUIS 11-05-15] @BOR
             * On vérifie si l'utilisateur est sur la page de la cible.
             * TODO : Récupèrer l'URL depuis les variables globales $_SESSION et $_SERVER
             */
            $TQR = new TRENQR($_SESSION["sto_infos"]->getProd_xmlscope());
            $upieces = $TQR->explode_tqr_url($this->KDIn["datas"]["cl"]);
            if ( $this->return_is_error_volatile(__FUNCTION__, __LINE__, $upieces) ) {
                $this->Ajax_Return("err",$upieces);
            } 
            
            $rld = ( !empty($upieces["user"]) && ( strtolower($upieces["user"]) === strtolower($exists["pdacc_upsd"]) ) ) ? TRUE : FALSE;
            $this->KDOut["FE_DATAS"] = [
                /*
                 * [NOTE 11-05-15] @BOR
                 * Indique à FE s'il faut recharger. On recharge dans le cas où CU est sur la page de l'utilisateur visé.
                 */
                "rld"   => $rld,
                "urel"  => ( $rld === TRUE ) ? $cdrl : NULL
            ];
