 ...
 */
class WORKER_TQR_TIA_MYSRY_VOTE extends WORKER  {
...
//            }
    ...
    private function Process() {
        ...
        $mmi = $this->KDIn["datas"]["mmi"];
        $vtc = $this->KDIn["datas"]["vtc"];
        
        
//        var_dump(__FILE__,__FUNCTION__,__LINE__,$mmi,$vtc);
//        exit();
        
        $MYSM = new MYSTERY();
        
        $votetab = $MYSM->onvote_add($this->KDIn["oid"],$mmi,$vtc,$this->KDIn["locip"],session_id(),$this->KDIn["datas"]["cu"],$this->KDIn["uagent"]);
        if ( $this->return_is_error_volatile(__FUNCTION__, __LINE__, $votetab) ) {
            $this->Ajax_Return("err",$votetab);
        }
        ...
        $votesum = $MYSM->onvote_sum($mmi);
        if ( $this->return_is_error_volatile(__FUNCTION__, __LINE__, $votesum) ) {
            $this->Ajax_Return("err",$votesum);
        }
        $FE = $votesum;
