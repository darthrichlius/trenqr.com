...
class WORKER_STGS_SUBT_PWD extends WORKER  {
...
//            }
    ...
    function SaveForm() {
        ...
        $TQCNX = new TQR_CONX();
        $IsConf = $TQCNX->checkPwdForUser($this->KDIn["datas"]["opwd"],$this->KDIn["oid"]);
        ...
        if ( $this->return_is_error_volatile(__FUNCTION__, __LINE__, $IsConf) ) {
            ...
            $args = [
                "accid"     => $this->KDIn["oid"],
                "ins_pwd"   => $this->KDIn["datas"]["npwd"],
                "locip"     => $this->KDIn["locip"]
            ];

            $TQACC = new TQR_ACCOUNT();
            $FE = $TQACC->onalter_password($args);
            ...
            if ( !is_array($FE) && $FE === "DONE" ) {
                ...
                //On met à jour SESSION, ca ne nous coute rien
                $PDACC = new PROD_ACC();
                ...
                //Le mot de passe a été changé. Il ne faut pas envoyer un message diffus juste parce qu'on a pas réussi une opération facultative.
//                if (! $sr ) {
