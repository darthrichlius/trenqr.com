...
class WORKER_STGS_SUBT_DELACC extends WORKER  {
...
//            }
    ...
    function SaveForm() {
        ...
        if ( $this->return_is_error_volatile(__FUNCTION__, __LINE__, $IsConf) ) {
            ...
            $args = [
                "accid"     => $this->KDIn["oid"],
                "hikw"      => $this->KDIn["datas"]["hikw"],
                "yilv"      => $this->KDIn["datas"]["yilv"],
                "yilv_ot"   => $this->KDIn["datas"]["yilv_ot"],
                "ilbbif"    => $this->KDIn["datas"]["ilbbif"],
                "locip"     => $this->KDIn["locip"]
            ];
//            var_dump($args);
//            exit();
            $TQACC = new TQR_ACCOUNT();
            $FE = $TQACC->ondelete_apply($args);
    ...
    public function prepare_datas_in() {
        ...
        $EXPTD = ["hikw","yilv","yilv_ot","ilbbif","cfpwd"];
        ...
        foreach ($in_datas_keys as $k => $v) {
            if ( !( isset($v) && ( $v !== "" || in_array($k, ["yilv_ot","ilbbif"]) ) ) )  {
                $this->Ajax_Return("err","__ERR_VOL_DATAS_MSG");
            }
        }
