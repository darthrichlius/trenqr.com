...
class WORKER_TMLNR_GTPG_RU extends WORKER {
...
//                    "msg" => addcslashes($article["art_desc"], "\0..\37!@\177..\377 \\"),
                    "prmlk"     => $_SERVER["HTTP_HOST"].$ART->onread_AcquierePrmlk($article["art_eid"],$ivid), //[DEPUIS 28-04-15]
...
//                    "trhref"    => $article["trd_href"], [28-04-15]
    ...
    private function AcquireUREL () {
        ...
        $target_id = $this->KDIn["target"]["pdaccid"];
        $arctor_id = $this->KDIn["curr"]["pdaccid"];
        
        //Détermine s'il y a une relation entre CU et OW
        $REL = new RELATION();
        $r = $REL->onread_relation_exists_fecase($arctor_id,$target_id);
...
//        exit();
    ...
    private function AcquireFavArtsDatas () {
        ...
        $arts = $ART->Favorite_GetFavArts($this->KDIn["target"]["pdaccid"], $this->KDIn["curr"]["pdaccid"], "FST", NULL, NULL, NULL, ["ONLY_PUB"]);
        ...
        if ( $this->return_is_error_volatile(__FUNCTION__, __LINE__, $arts) ) {
            ...
            foreach ($arts as $article) {
                ...
                $atab = [
                    ...
                    "hasfv"     => $ART->Favorite_hasFavorite($this->KDIn["curr"]["pdaccid"], $article["art_eid"]),
                    /*
                     * [DEPUIS 21-04-16]
                     */
                    "vidu"      => $article["art_vid_url"],
...
//        exit();
    ...
    */
    ...
    public function prepare_datas_in() {
        ...
        //On vérifie bien que la demande ne concerne pas CU. Auquel cas il faut faire une redirection
        if ( strtolower($target_id) === strtolower($curr_user_id) ) {
            //[NOTE 26-08-14] Suivant les modofications entreprises, ce cas strictement IMPOSSIBLE. Il serivra cependant de garde fou 'PARANOIAQUE'
//            $this->signalErrorWithoutErrIdButGivenMsg("MUST_REDIR_To_OWN : WATCHMAN", __FUNCTION__, __LINE__);
//            exit();
                
            $R = new REDIR($STOI->getProd_xmlscope());
            $url = $R->redir_to_default_page("pdpage_rest_own_pgid");
            
            $TXH = new TEXTHANDLER();
            $url = $TXH->ReplaceDmd("pseudo", strtolower($RSTOI->getUpseudo()), $url);
            
            $R->start_redir_to_this_url_string($url);
        } 
        
        //TODO : Vérifier la relation entre les utilisateurs pour rediriger vers RF ou RFOL
...
//        exit();
        ...
        if ( $this->KDIn["curr"]["pdaccid"] ) {
            ...
            switch ($sector) {
                case "ml" :
                        $log_r = $this->Wkr_LogUsertagActy($PM, $this->KDIn["curr"]["pdaccid"], $_SESSION['sto_infos']->getCurrent_ipadd(), $this->KDIn["srv_curl"], $this->KDIn["srv_curl"], FALSE, $this->KDIn["target"]["pdaccid"], 115, TRUE);
                    break;
                case "tr" :
                        $log_r = $this->Wkr_LogUsertagActy($PM, $this->KDIn["curr"]["pdaccid"], $_SESSION['sto_infos']->getCurrent_ipadd(), $this->KDIn["srv_curl"], $this->KDIn["srv_curl"], FALSE, $this->KDIn["target"]["pdaccid"], 116, TRUE);
                    break;
                case "fv" :
                        $log_r = $this->Wkr_LogUsertagActy($PM, $this->KDIn["curr"]["pdaccid"], $_SESSION['sto_infos']->getCurrent_ipadd(), $this->KDIn["srv_curl"], $this->KDIn["srv_curl"], FALSE, $this->KDIn["target"]["pdaccid"], 117, TRUE);
                    break;
            }
        }
...
//        $this->presentVarIfDebug(__FUNCTION__, __LINE__, $_SESSION["ud_carrier"],'v_d');
//        exit();
    }
