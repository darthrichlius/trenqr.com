...
class WORKER_CHBX_PL_MS extends WORKER  {
...
//                        "cmsgm" => html_entity_decode($datas["chmsg_msg"]),
            ...
            case "cmsg_from_convrs":
                    $nds = [
                        ...
                        /*
                         * [DEPUIS 11-11-15] @author BOR
                         *      Je ne sais pas comment traité ce cas. Aussi, je renvoi NULL
                         */
                        "cmsgrd"    => NULL,
//                        "cmsgrd"    => $datas["conv_lmsgtab"]["chmsg_rdate_tstamp"],
    ...
    public function PullMessages() {
         ...
         * 
         * ATTENTION : Il ne faut renvoyer ces données qu'accompagnée d'une donnée qui confirme à FE si l'utilisateur est sur une page de type OWNER
         */
        $PA = new PROD_ACC();
...
//        var_dump(__LINE__,$this->KDIn["datas"]["curl"],$upieces);
//        exit();
            ...
            */
            ...
            do {
                if ( isset($lmi) && !empty($lmi) ) {
                    //CAS : Pomper les Messages d'une Conversation à partir d'un message pivot. Il s'agit surtout du cas d'une mise à jour de la Conversation (Message)

                    //Récupération des messages depuis lmi
                    $plist = $this->GiveMeFrom($cid,$lmi,$drt);
                } else {
                    //CAS : Pomper les Messages d'une Conversation dont le pivot n'est pas disponible. Il s'agit 
...
//                    $cmlist[] = $this->OnlyMyDatas("cmsg_from_cmsg_lightway",$v);
    ...
    public function prepare_datas_in() {
         ...
         * "flm"  : FiLterMenu,
         * "cid"  : ConversationID,
         * "lmi"  : LastMessageId, Permet de définir le mode de distribution. "First" ou "From"
         * "drt"  : DiRecTion
         * "pcf"  : PullConF
         ...
         */
        $EXPTD = ["tgt","flm","cid","lmi","drt","pcf","wso","curl"];
