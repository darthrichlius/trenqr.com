...
class WORKER_FKSA_GTPG_VIDVER extends WORKER {
...
//        exit();
    ...
    private function CustumHeaderDatas () {
         ...
         * [DEPUIS 04-07-16]
         */
        if ( defined("WOS_MAIN_HTTPS_HOST")  && WOS_MAIN_HTTPS_HOST ) {
            $this->KDout["head"]["apichrf"] = WOS_MAIN_HTTPS_HOST.$this->KDOut["ART_TAB"]["art_pdpic_path"];
        } else {
            $this->KDout["head"]["apichrf"] = $this->KDOut["ART_TAB"]["art_pdpic_path"];
        }
        
        /*
         * [DEPUIS 18-05-16] @author BOR 
         */
        $vidu = $this->KDOut["ART_TAB"]["art_vidu"];
         ...
         */
        if ( defined("WOS_MAIN_HTTPS_HOST") && WOS_MAIN_HTTPS_HOST ) {
            $n_vidu = html_entity_decode($matches[1]);
            $this->KDout["head"]["avidu"] = WOS_MAIN_HTTPS_HOST.$n_vidu;
        } else {
            $n_vidu = html_entity_decode($matches[1]);
            $this->KDout["head"]["avidu"] = $n_vidu;
        }
        
        $this->KDout["head"]["vidw"] = intval($width);
        $this->KDout["head"]["vidh"] = intval($height);
    }
...
//        exit();
    ...
    public function prepare_datas_in() {
        ...
        if (! ( key_exists("atypi", $this->KDIn["DsIn"]) && !empty($this->KDIn["DsIn"]["atypi"]) 
            && is_string($this->KDIn["DsIn"]["atypi"]) && in_array(strtolower($this->KDIn["DsIn"]["atypi"]), ["video"] ) 
        ) ) {
            $this->signalError ("err_user_l5e404_any", __FUNCTION__, __LINE__);
        }
...
//                var_dump(__LINE__,base64_encode(serialize($v)));
        ...
        if ( $this->KDIn["curr"]["pdaccid"] ) {
            $PM = new POSTMAN();
            $log_r = $this->Wkr_LogUsertagActy($PM, $this->KDIn["curr"]["pdaccid"], $_SESSION['sto_infos']->getCurrent_ipadd(), $this->KDIn["srv_curl"], $this->KDIn["srv_curl"], FALSE, $this->KDOut["ART_TAB"]["artid"], 1502, TRUE);
        }
