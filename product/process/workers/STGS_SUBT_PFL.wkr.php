...
class WORKER_STGS_SUBT_PFL extends WORKER  {
...
//            }
    ...
    function SaveForm() {
        ...
        if ( $this->return_is_error_volatile(__FUNCTION__, __LINE__, $IsConf) ) {
            ...
            $args = [
                "accid" => $this->KDIn["oid"],
                "ins_fn" => $this->KDIn["datas"]["fullname"],
                "ins_nais_tstamp" => $this->KDIn["datas"]["birthdate_tsp"],
                "ins_gdr" => $this->KDIn["datas"]["gender"],
                "ins_cty" => $this->KDIn["datas"]["city"],
                "locip" => $this->KDIn["locip"]
            ];

            $TQACC = new TQR_ACCOUNT();
            $FE = $TQACC->onalter_profile($args);
            if ( $this->return_is_error_volatile(__FUNCTION__, __LINE__, $FE) ) {
                $this->Ajax_Return("err",$FE);
            }

            if ( is_array($FE) && ( $FE[0] === "DONE" || $FE[0] === "FAILED" ) ) {
    ...
    public function prepare_datas_in() {
        ...
        $EXPTD = ["fullname","birthdate_tsp","gender","city","cfpwd"];
