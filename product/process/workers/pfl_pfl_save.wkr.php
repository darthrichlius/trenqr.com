...
class WORKER_pfl_pfl_save extends WORKER  {
    ...
    private function pfl_profileSave($args){
        $PFL = new PROFIL();
        $pflid = $PFL->get_profile_from_account($args['accid']);
        $data = [
            'pflid' => $pflid,
            'ufullname' => $args['fullname'],
            'uborndate' => $args['birthday'],
            'ugender' => $args['gender'],
            'ulvcity' => $args['cityId']
        ];
        $e = $PFL->on_alter_entity($data);
        if($e == 1){
    ...
    public function on_process_in() {
        $rt = $this->pfl_profileSave($this->KDIn['datas']);
        $this->KDOut["rt"] = $rt;
    }
