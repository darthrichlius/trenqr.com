...
//            exit();
         ...
         * De plus, cela ne DEVRAIT pas dénaturé le pseudo donné enparamètre
         */
...
//        exit();
    ...
    private function AcquireOwnerDatas () {
        ...
        $A = new PROD_ACC();
        $loads = $A->on_read_entity(["accid" => $accid]);
...
//        $art_stack = $A->onread_load_my_first_articles($accid);
         ...
         */
        $art_stack = $A->onread_load_my_first_articles($accid,["VM_ART"]);
...
//                    "msg" => addcslashes($article["art_desc"], "\0..\37!@\177..\377 \\"),
                    "prmlk"     => $_SERVER["HTTP_HOST"].$ART->onread_AcquierePrmlk($article["art_eid"]),
...
//        exit();
        ...
        foreach ($din as $k => $v) {
            if ( empty($v) | !in_array(strtolower($din["pg"]), ["ml","tr"]) ) {
                $this->presentVarIfDebug(__FUNCTION__, __LINE__, ["WE GOT => ",$din],'v_d');
                $this->signalError("err_sys_l7comn1",__FUNCTION__, __LINE__);
            }
        }
