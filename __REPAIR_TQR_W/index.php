...
//* 
/*
 * [DEPUIS 14-09-15] @author
 *  L'objectif est de permettre créer une bulle de session entre les machines "dm1606" et "edge-*".
 *  De plus, personnaliser le nom du cookie de session fait moins amateur.
 * [GLOSSAIRE]
 *  sitting : Fait référence au mot 'session'. Il est proche de 'séance'
 */ 
session_name("_tqr_sitting");
...
/*
session_start(); 
var_dump(__LINE__,session_get_cookie_params(),session_id(),session_name());
session_destroy();
var_dump(__LINE__,session_get_cookie_params(),session_id(),session_name());
 ...
 */
define("WOS_THIS_SUBDOMAIN","www");
define("WOS_THIS_SUBDOMAIN_ROOTPA","/www");
/*
...
if (! empty($complement) ) {
    ...
    define("HTTP_RACINE", "https://".$_SERVER['HTTP_HOST'].'/'.$complement);
} else {
    define("RACINE", $_SERVER['DOCUMENT_ROOT']);
    define("HTTP_RACINE", "https://".$_SERVER['HTTP_HOST']);
}
/*
